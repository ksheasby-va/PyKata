...
def map_sleeping_minutes(input):
    times = []
    for i in input.splitlines():
        time, value = datetime.datetime.strptime(i.split("]")[0].replace("[", ""), "%Y-%m-%d %H:%M"), i.split("]")[1].strip()
        times.append({"time": time, "value": value})
    times = sorted(times, key=lambda t: t["time"])
    guard, sleep_time, wake_time = None, None, None
    sleep_times = {}
    print times
    for e in times:
        if "begins shift" in e["value"]:
            guard = int(e["value"].split()[1].replace("#", ""))
        elif "falls asleep" in e["value"]:
            sleep_time = e["time"]
        elif "wakes up" in e["value"]:
            wake_time = e["time"]
        ...
        if sleep_time and wake_time:
            while sleep_time < wake_time:
                if not sleep_times.get(guard):
                    sleep_times[guard] = {}
                if sleep_times.get(guard).get(sleep_time.minute) is None:
                    sleep_times[guard][sleep_time.minute] = 1
                else:
                    sleep_times[guard][sleep_time.minute] += 1
...
def most_asleep(totals):
    return max(totals.iteritems(), key=operator.itemgetter(1))[0]


def most_slept_minute(totals, guard):
    return max(totals[guard].iteritems(), key=operator.itemgetter(1))[0]

def foo(input):
    sleep_times = map_sleeping_minutes(input)
    totals = get_total_asleep_time(sleep_times)
    asleep = most_asleep(totals)
    max_min = most_slept_minute(sleep_times, asleep)
    return asleep * max_min


def get_total_asleep_time(sleep_times):
    totals = {}
    for guard, times in sleep_times.iteritems():
        total = 0
        for t in times.itervalues():
            total += t
        totals[guard] = total
    return totals
...
if __name__ == "__main__":
    print foo("""[1518-10-31 00:58] wakes up
