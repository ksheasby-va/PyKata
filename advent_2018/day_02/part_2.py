...
def find_ids_1_letter_apart(ids, comp):
    for i, s in enumerate(ids):
        if s != comp:
            diff = difflib.ndiff(comp, ids[i])
        else:
            continue
        if len(list(diff)) > len(s) + 1:
            continue
        return comp, s
...
def find_common_letters(ids):
    id_list = ids.split("\n")
    comp, found = None, None
    for i, s in enumerate(id_list):
        comp, found = find_ids_1_letter_apart(id_list, s)
        if found:
            break
    ret_val = ""
...
if __name__ == '__main__':
    print find_common_letters("""myhposlqgeauywfikztndcvrqr
